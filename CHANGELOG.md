...
### Removed
- **"Mark as Read" Action**: Removed the `read` action from the automation engine. This action was redundant in most workflows (as archiving/deleting is the primary goal) and often conflicted with other state changes. Existing rules will have this action automatically removed via migration.

### Fixed
- **Action Constraints**: Relaxed database constraints on the legacy `action` column to support a wider range of action types (labels, folders, stars) without validation errors during rule creation.
...
### Added
- **Update Notification**: Added a "New Version Available" banner that automatically detects when a newer version of the application is published to npm. This solves the issue where `npx` caches stale versions, ensuring users are always prompted to use the latest features.
- **Version Check Utility**: Implemented a smart version checker that periodically polls the npm registry (throttled to every 5 minutes) to detect updates without impacting performance.
...
### Fixed
- **LLM Configuration UI**: Improved the reliability of the provider/model dropdowns. selections now persist correctly during loading states, and saved values are always preserved even if discovery fails.
- **Connection Testing**: The "Test Connection" button now uses the currently selected (unsaved) provider/model values, allowing users to verify settings before saving.
- **SDK Fallbacks**: Implemented robust fallback strategies for when the SDK is unavailable or returns empty provider lists, ensuring the app always has a working default configuration (`realtimexai` / `gpt-4o-mini`).
- **UI Polish**: Cleaned up the Account Settings page layout and Logout button placement.
...
### Added
- **Updated Developer Architecture Docs**: Fully documented the new backend orchestration engine. Added details on bundled migration assets, portable CLI strategies, and the consolidated `migrate.sh` workflow.
...
### Fixed
- **Token Management**: Resolved "Invalid or expired token" errors by ensuring the Supabase session is automatically refreshed before making backend API calls.
- **Outlook Sync Reliability**: Implemented background token refreshing for Microsoft accounts. Previously, Outlook sync would fail after the initial access token expired.
- **Improved Scopes**: Added `offline_access` to Microsoft authentication to ensure long-lived synchronization is possible.
- **Better Error Reporting**: Account sync now provides clearer, provider-specific error messages (e.g., "Gmail Sync Error" vs "Outlook Sync Error") to aid in troubleshooting.

        ...
        <div className="flex-1 flex flex-col justify-center space-y-8" onKeyDown={handleKeyDown}>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-muted/40 border border-border/50 flex flex-col gap-2 group hover:bg-primary/5 hover:border-primary/20 transition-all">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest">{t('setup.encryptedTitle')}</p>
                    <p className="text-[10px] text-muted-foreground">{t('setup.encryptedDesc')}</p>
                </div>
                <div className="p-4 rounded-2xl bg-muted/40 border border-border/50 flex flex-col gap-2 group hover:bg-primary/5 hover:border-primary/20 transition-all">
                    <Zap className="w-5 h-5 text-primary" aria-hidden="true" />
                    <p className="text-[10px] font-bold uppercase tracking-widest">{t('setup.turboTitle')}</p>
                    <p className="text-[10px] text-muted-foreground">{t('setup.turboDesc')}</p>
                </div>
            </div>
